package com.example.tareasyncrona.services.api;

import com.androidnetworking.AndroidNetworking;
import com.androidnetworking.common.ANRequest;
import com.androidnetworking.common.ANResponse;
import com.example.tareasyncrona.Constants;
import com.example.tareasyncrona.data.api.ProductRMI;
import com.example.tareasyncrona.data.api.ResponseDataWithCode;
import com.example.tareasyncrona.data.api.ResponseList;
import com.example.tareasyncrona.domain.services.interfaces.ProductRMIService;
import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.reflect.TypeToken;

import java.util.ArrayList;

public class ProductRMIApiServiceImpl implements ProductRMIService {

    private static ProductRMIApiServiceImpl instance;

    private ProductRMIApiServiceImpl() {
    }

    public static ProductRMIApiServiceImpl getInstance() {
        if (instance == null)
            instance = new ProductRMIApiServiceImpl();
        return instance;
    }

    @Override
    public ResponseDataWithCode<ArrayList<ProductRMI>> fetch() {
        String URL = Constants.Api.URL.getIP() + "/api/listprice/1/rmi";
        ANRequest request = AndroidNetworking.get(URL)
                .build();

        ANResponse<ResponseList<ProductRMI>> response = request.executeForObject(ResponseList.class);

        ArrayList<ProductRMI> productRMIS = null;
        String message;

        if (response.isSuccess() && response.getResult().getData() != null) {
            productRMIS = new GsonBuilder()
                    .create()
                    .fromJson(new Gson().toJsonTree(response.getResult().getData()),
                            new TypeToken<ArrayList<ProductRMI>>() {
                            }.getType());
            message = response.getResult().getMessage();
        } else {
            message = response.getError().getMessage();
        }

        if (response.getOkHttpResponse() == null) {
            return new ResponseDataWithCode<>(productRMIS, 102, message);
        }

        return new ResponseDataWithCode<>(productRMIS, response.getOkHttpResponse().code(), message);
    }

    @Override
    public void addList(ArrayList<ProductRMI> productRMIS) {

    }

    @Override
    public void addObject(ProductRMI productRMI) {

    }
}
